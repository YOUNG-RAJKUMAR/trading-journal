function getSessionHighLowStudy(PineJS) {
    const Study = function() {
        this._sessionData = {
            asia: { high: NaN, low: NaN, isActive: false, isComplete: false, broken: { high: false, low: false }, startBar: null },
            london: { high: NaN, low: NaN, isActive: false, isComplete: false, broken: { high: false, low: false }, startBar: null },
            newyork: { high: NaN, low: NaN, isActive: false, isComplete: false, broken: { high: false, low: false }, startBar: null },
        };
        this._prevDay = null;
        this._barIndex = 0;
    };
    ...
    Study.prototype.main = function(context, inputCallback) {
        ...
        if (this._prevDay !== currentDay) {
            ...
            Object.keys(this._sessionData).forEach(k => {
                ...
                this._sessionData[k].isComplete = false;
        ...
        const sessions = [
            { id: 'asia', start: parseTime(this._input(2)), end: parseTime(this._input(3)), label: 'A' },
            { id: 'london', start: parseTime(this._input(4)), end: parseTime(this._input(5)), label: 'L' },
            { id: 'newyork', start: parseTime(this._input(6)), end: parseTime(this._input(7)), label: 'N' }
        ];
        ...
        const getVal = (id, type) => {
            ...
            // If broken, stop drawing the line
            if (d.broken[type]) return NaN;
            // If session is active, we draw it. If complete, we also draw it (as a level).
            return type === 'high' ? d.high : d.low;
        };

        const getShape = (id) => {
            const d = this._sessionData[id];
            // Only show label at the start of the session
            if (d.isActive && d.startBar === this._barIndex) {
                return d.high;
            }
            return NaN;
        };
