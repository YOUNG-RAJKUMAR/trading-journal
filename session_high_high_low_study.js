function getSessionHighLowStudy(PineJS) {
    const Study = function() {
        this._sessionData = {
            asia: { high: NaN, low: NaN, highBar: null, lowBar: null, isActive: false, isComplete: false, broken: { high: false, low: false }, startBar: null },
            london: { high: NaN, low: NaN, highBar: null, lowBar: null, isActive: false, isComplete: false, broken: { high: false, low: false }, startBar: null },
            newyork: { high: NaN, low: NaN, highBar: null, lowBar: null, isActive: false, isComplete: false, broken: { high: false, low: false }, startBar: null },
        };
        this._prevDay = null;
        this._barIndex = 0;
    };
    ...
    Study.prototype.main = function(context, inputCallback) {
        ...
        if (this._prevDay !== dayId) {
            ...
            Object.keys(this._sessionData).forEach(k => {
                ...
                this._sessionData[k].highBar = null;
                this._sessionData[k].lowBar = null;
        ...
        sessions.forEach(s => {
            ...
            if (inSession) {
                ...
                // Break detection
                if (!isNaN(data.high) && !data.broken.high && context.symbol.high > data.high) {
                    data.broken.high = true;
                    data.highBreakBar = this._barIndex;
                }
                if (!isNaN(data.low) && !data.broken.low && context.symbol.low < data.low) {
        ...
        const getVal = (id, type) => {
            ...
            // To ensure perfectly horizontal lines like TradingView, we only draw 
            // the lines AFTER the session is complete.
            if (d.isActive || isNaN(d.high) || d.day !== dayId) return NaN;
            ...
            let lineEndBar = d.endBar + extDist;
            if (isBroken && breakBar) {
                lineEndBar = breakBar;
            }
            
            if (startBar && this._barIndex >= startBar && this._barIndex <= lineEndBar) {
