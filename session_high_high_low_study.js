function getSessionHighLowStudy(PineJS) {
    const Study = function() {
        this._sessionData = {
            asia: { high: NaN, low: NaN, isActive: false, broken: { high: false, low: false }, startBar: null },
            london: { high: NaN, low: NaN, isActive: false, broken: { high: false, low: false }, startBar: null },
            newyork: { high: NaN, low: NaN, isActive: false, broken: { high: false, low: false }, startBar: null },
        };
        this._prevDay = null;
        this._barIndex = 0;
    };
    ...
    Study.prototype.main = function(context, inputCallback) {
        ...
        // Input indices: 0 is lineWidth, 1 is asiaStart, 2 is asiaEnd...
        const sessions = [
        ...
        const getVal = (id, type) => {
            ...
            // Only draw while session is active OR if it's the current day and not broken
            if (d.broken[type]) return NaN;
            // To prevent connecting between days, we return NaN if the session is not active 
            // and we are far from the session end (optional, but isActive check is safer)
            // However, to keep the level visible after session ends (like TV), we return the value.
            // The "Step Line" plot type will handle the horizontal part.
    ...
    return {
        ...
        metainfo: {
            ...
            styles: {
                // plottype 2 is Step Line (Horizontal + Vertical jumps)
                // plottype 8 is Line With Breaks (Correct for NaN handling)
                // We use 2 for the "Step" look, but 8 is often better for breaks. 
                // Let's use 2 and ensure we return NaN correctly.
